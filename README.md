...
- Uyumsoft inbound sync now continues through the canonical import pipeline for eligible Inbox supplier invoices: read-only provider list/download/persistence, existing UBL parser, `InternalInvoice`, `ImportInvoiceCommand`, `ImportInvoiceUseCase`, Hub review/evidence or technical import receipt idempotency, and optional Odoo Workbench projection. Successful non-review `dry_run` results are reported as canonical acceptance/evaluation only; they do not create ERP documents.
...
- Odoo Online Workbench projection contract and publisher for configured Odoo Studio review display and explicit decision capture.
...
- Explicit Workbench Vendor Bill execution trigger at `POST /api/workbench/reviews/{review_id}/execute`, scoped to persisted accepted direct Vendor Bill decisions and pinned Hub execution source evidence.
...
- Workbench execution through the explicit `/execute` trigger for any decision other than direct Vendor Bill without business-context allocations. RFQ, Purchase Order, Expense, Asset, Subscription, Customer Invoice, Customer Recharge, Project Cost, Internal Cost, Dismiss, and Manual Review execution remain unsupported there.
