...
- Uyumsoft inbound sync now continues through the canonical import pipeline for eligible Inbox supplier invoices: read-only provider list/download/persistence, existing UBL parser, `InternalInvoice`, `ImportInvoiceCommand`, `ImportInvoiceUseCase`, Hub review/evidence, and optional Odoo Workbench projection.
