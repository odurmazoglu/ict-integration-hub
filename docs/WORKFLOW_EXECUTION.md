...
After successful runtime execution or replay of an already completed direct Vendor Bill decision, `WorkbenchVendorBillExecutionWorkflow` invokes `OdooWorkbenchProjectionPublisher.project_vendor_bill_execution_result(...)` to project the execution outcome (`execution_status`, `execution_id`, `execution_mode`, `execution_runtime_state`, `vendor_bill_id`, `vendor_bill_external_identity`, `vendor_bill_created`, `execution_message`, `last_sync_at`, `trace_id`) back to the configured Odoo Workbench record. If projection publishing fails, Hub execution remains authoritative; subsequent execution replay repairs the projection without re-executing the runtime or creating a second Vendor Bill in Odoo.
