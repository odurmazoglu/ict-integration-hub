...
For operator-resolved Vendor Bills (P0-PROD-08M), Stage 1 also preserves partial matching facts when the deterministic workflow is `manual_review`, the supplier and all taxes match, and the invoice carries product identifiers. An unresolved product remains unresolved in Stage 1. At acceptance, explicit `selected_product_id` selections are validated read-only and substituted only into the derived Stage 2 product match; explicit `account_only` selections require a validated company-scoped `expense_account_id`, pinned in the accepted decision's existing `line_resolutions` JSON. Every fresh Vendor Bill decision must pass `validate_vendor_bill_inputs` after these resolutions, and unknown resolution lines are rejected. Existing decision replay keeps its original contract and still verifies immutable evidence equality. Neither the source snapshot nor the classification changes as a result of operator resolution. A Stage 1 row (including the reclassification API's historical `executable` flag) means evidence is available for validation, not permission to execute. See [the investigation and pilot limitation](P0_PROD_08M_EXECUTION_EVIDENCE_GAP.md).
...
Odoo Workbench billing authoring is the authoritative Stage 1 capture source for Customer Invoice creation terms. Hub capture reads exact Odoo child rows through a read-only adapter, validates review/company/version alignment, validates exact creation-mode `CUSTOMER_RECHARGE` allocation coverage, validates exact ERP references, and persists immutable `CustomerInvoiceBillingInstruction` evidence. Missing, stale, malformed, cross-company, incomplete, duplicated, inferred, or ERP-reference-invalid billing rows fail closed before accepted decision persistence, Stage 2 pinning, runtime creation, or writer calls.
...
After successful runtime execution or replay of an already completed direct Vendor Bill decision, `WorkbenchVendorBillExecutionWorkflow` invokes `OdooWorkbenchProjectionPublisher.project_vendor_bill_execution_result(...)` to project the minimal execution outcome (`execution_status`, `vendor_bill`, `execution_message`, `last_sync_at`, `trace_id`) back to the configured Odoo Workbench record. `execution_id`, `execution_mode`, `execution_runtime_state`, `vendor_bill_external_identity`, and `vendor_bill_created` remain intentionally out of the required user-facing contract because they are support-only runtime details and are not required in the Workbench UX. If projection publishing fails, Hub execution remains authoritative; subsequent execution replay repairs the projection without re-executing the runtime or creating a second Vendor Bill in Odoo.
