...
Customer Invoice Stage 2 pinning and execution wiring complete; authoritative Stage 1 billing instruction capture source remains required. Repository inspection shows the normal `InternalInvoice` import path delegates to `ImportInvoiceUseCase -> DecisionEngine` and the current Workbench review creation path uses `ReviewItemCreationService.create_pending_review_item_with_execution_evidence(...)` for Vendor Bill-capable reviews. There is no production caller that supplies `ReviewExecutionBillingEvidence` or `CustomerInvoiceBillingInstruction` to `create_review_item_with_billing_evidence(...)` or `create_review_item_with_execution_and_billing_evidence(...)`. The system must therefore fail closed for Customer Invoice creation allocations until a dedicated Customer Billing Instruction Capture / Workbench Authoring source exists.
