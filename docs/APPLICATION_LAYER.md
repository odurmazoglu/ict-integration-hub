...
`ImportInvoiceUseCase` now has optional Workbench runtime collaborators. The production composition entrypoint is `app/composition/imports.py::build_import_invoice_use_case`, and source guards keep direct `ImportInvoiceUseCase(...)` construction out of other `app/` runtime paths. The Uyumsoft inbound path, reachable only when `UYUMSOFT_SYNC_EXECUTE_ENABLED=true` (default `false`; see ADR-0007) -- independently of which `UYUMSOFT_ENVIRONMENT` is configured -- is `sync_uyumsoft_invoices -> UyumsoftInvoiceSyncWorkflow -> UyumsoftCanonicalInvoiceImporter -> parse_ubl_invoice(...) -> InternalInvoice -> ImportInvoiceCommand -> build_import_invoice_use_case(...) -> ImportInvoiceUseCase.execute(...)`. The canonical importer resolves company context exactly from the UBL buyer tax number through read-only Odoo company lookup; unresolved, missing, ambiguous, or expected ERP repository lookup failures fail closed before canonical import. The Uyumsoft-triggered command uses `dry_run=True`: success means canonical evaluation accepted or planned the import path, not ERP execution. Non-review dry-run acceptance is recorded in the technical `import_receipts` table for deterministic replay; review-required imports remain idempotent through `workbench_review_items`. In production composition, review-required imports create the Hub `ReviewItem` and pinned `ReviewClassificationEvidence`, commit that Hub transaction through `UnitOfWork`, and only then attempt `WorkbenchProjectionPublisher.publish_projection(...)`. Projection publish failures are best-effort warnings on the import result only for `WorkbenchCandidateReadError`, `WorkbenchCandidateAmbiguityError`, and `WorkbenchProjectionPublishError`; unexpected bugs and contract/data-integrity failures propagate. Expected `ApplicationError` import failures may be converted by the external Uyumsoft trigger into safe per-item outcomes, while unexpected `TypeError`, `AssertionError`, `RuntimeError`, and similar defects propagate. Tolerated failures do not roll back Hub persistence, change the business classification, or re-run current rules. The composed Odoo publisher reads classification details through `WorkbenchClassificationProjectionService` and `ReviewClassificationEvidenceReader`.
