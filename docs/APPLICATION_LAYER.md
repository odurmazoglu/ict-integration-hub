...
`ImportInvoiceUseCase` now has optional Workbench runtime collaborators. The production composition entrypoint is `app/composition/imports.py::build_import_invoice_use_case`, and source guards keep direct `ImportInvoiceUseCase(...)` construction out of other `app/` runtime paths. Repository tracing shows no externally reachable HTTP route, CLI command, scheduled job, bootstrap path, or Uyumsoft inbound handler currently calls that builder and then invokes `ImportInvoiceUseCase.execute(...)`. This slice provides production-ready composition and runtime-capable wiring only; attaching it to an external normalized invoice import trigger remains follow-up. The future integration point is the first adapter that owns a canonical `InternalInvoice` and idempotency key. For the existing Uyumsoft sync surface, that follow-up belongs after `sync_uyumsoft_invoices -> UyumsoftInvoiceSyncWorkflow -> InvoicePersistenceService` has persisted or fetched provider data and a normalization/import adapter is ready to create `ImportInvoiceCommand`. In production composition, review-required imports create the Hub `ReviewItem` and pinned `ReviewClassificationEvidence`, commit that Hub transaction through `UnitOfWork`, and only then attempt `WorkbenchProjectionPublisher.publish_projection(...)`. Projection publish failures are best-effort warnings on the import result only for `WorkbenchCandidateReadError`, `WorkbenchCandidateAmbiguityError`, and `WorkbenchProjectionPublishError`; unexpected bugs and contract/data-integrity failures propagate. Tolerated failures do not roll back Hub persistence, change the business classification, or re-run current rules. The composed Odoo publisher reads classification details through `WorkbenchClassificationProjectionService` and `ReviewClassificationEvidenceReader`.
