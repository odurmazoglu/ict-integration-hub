...
The Uyumsoft inbound sync endpoint is now the live external import trigger for eligible incoming supplier invoices, once explicitly authorized: `app/api/routers/uyumsoft_sync.py::sync_uyumsoft_invoices(...)` fails closed with `404` unless `UYUMSOFT_SYNC_EXECUTE_ENABLED=true` (default `false`; see ADR-0007), independently of which `UYUMSOFT_ENVIRONMENT` is configured. The `from`/`to` date range (plus `direction`/`page_size`/`max_pages`) controls what the provider returns; an optional exact `invoice_ettn` allowlist additionally controls which of those returned invoices are persisted and imported -- omitting it preserves normal batch sync behavior, while supplying it enables a deterministic, controlled single-invoice (or small-set) execution without any separate code path. Once authorized, it drives `UyumsoftInvoiceSyncWorkflow`, which first preserves provider metadata, then uses `UyumsoftCanonicalInvoiceImporter` to download/read exactly one UBL XML document, call the existing `parse_ubl_invoice(...)` parser, create an `InternalInvoice`, resolve the exact Hub/Odoo company by buyer tax number, build `ImportInvoiceCommand`, resolve dependencies through `build_import_invoice_use_case(...)`, and call `ImportInvoiceUseCase.execute(...)`. The command remains `dry_run=True`; the import composition never creates an ERP document. A deterministically matched supplier invoice is routed to Workbench review by `VendorBillReviewRecommendationStrategy` — `workflow=VENDOR_BILL`, `review_required=True`, no `VendorBillBuilder` or Odoo writer call — and the draft `account.move` is created only later from a human-accepted `SELECT_WORKFLOW + VENDOR_BILL` decision through the existing execution runtime. Successful non-review results (currently only `MANUAL_REVIEW` recommendations that resolve without a review item, and duplicates) mean canonical acceptance/evaluation only; those persist a technical `import_receipts` row keyed by company-scoped idempotency key so repeated syncs return `already_imported`. Review creation persists immutable `ReviewClassificationEvidence` into `workbench_review_classification_evidence` and, for a fully matched Vendor Bill candidate, immutable Stage-1 `ReviewExecutionEvidence` (`InternalInvoice` + supplier/product/tax match snapshots) into `workbench_review_execution_evidence` — atomically with `workbench_review_items` — keyed by exact `company_id`, `review_id`, and `review_version`. Incomplete or ambiguous matches remain fail-closed: a review item is still created, but with no executable Vendor Bill Stage-1 evidence. When `ODOO_WORKBENCH_PROJECTION_PUBLISH_ENABLED=true`, the composed use case commits Hub review/evidence first and then attempts best-effort Odoo projection publishing through `WorkbenchProjectionPublisher`.
...
- synchronized invoices and documents
