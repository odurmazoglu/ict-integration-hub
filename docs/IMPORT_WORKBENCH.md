...
The import use case now creates a pending Hub Workbench review for review-required results when the production composition root supplies `ReviewItemCreationService`. The production composition entrypoint is `build_import_invoice_use_case(...)` in `app/composition/imports.py`; repository tracing shows there is currently no externally reachable HTTP route, CLI command, scheduled job, bootstrap path, or Uyumsoft inbound handler that calls this builder and then invokes `ImportInvoiceUseCase.execute(...)`. This PR therefore provides production-ready composition and runtime-capable wiring, but it does not yet attach that wiring to an external import trigger. Review creation persists immutable `ReviewClassificationEvidence` into `workbench_review_classification_evidence`, keyed by exact `company_id`, `review_id`, and `review_version`. When `ODOO_WORKBENCH_PROJECTION_PUBLISH_ENABLED=true` and a future import trigger uses this builder, the composed use case commits that Hub review/evidence first and then attempts best-effort Odoo projection publishing through `WorkbenchProjectionPublisher`.

The exact future integration point is the first external normalized invoice import trigger that owns a canonical `InternalInvoice` and idempotency key. For the existing Uyumsoft surface, that means after `app/api/routers/uyumsoft_sync.py::sync_uyumsoft_invoices` drives `UyumsoftInvoiceSyncWorkflow` and `InvoicePersistenceService` has persisted or fetched provider invoice data, a future normalization/import adapter must parse the invoice into `InternalInvoice`, construct `ImportInvoiceCommand`, resolve dependencies through `build_import_invoice_use_case(...)`, and call `ImportInvoiceUseCase.execute(...)`. The current Uyumsoft sync endpoint remains read-only provider synchronization and does not perform that canonical import step.
...
The production composition root wires this publisher after Hub review persistence when `ODOO_WORKBENCH_PROJECTION_PUBLISH_ENABLED=true`; no externally reachable import trigger invokes that composed use case yet. Once a future import trigger attaches to `build_import_invoice_use_case(...)`, the ordering is Hub review plus classification evidence commit first, Odoo projection attempt second. A projection failure returns a safe warning on the import result and does not roll back, downgrade, or reclassify the Hub review.
