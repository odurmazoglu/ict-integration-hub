...
The Uyumsoft inbound sync endpoint is now the live external import trigger for eligible incoming supplier invoices. `app/api/routers/uyumsoft_sync.py::sync_uyumsoft_invoices(...)` drives `UyumsoftInvoiceSyncWorkflow`, which first preserves provider metadata, then uses `UyumsoftCanonicalInvoiceImporter` to download/read exactly one UBL XML document, call the existing `parse_ubl_invoice(...)` parser, create an `InternalInvoice`, resolve the exact Hub/Odoo company by buyer tax number, build `ImportInvoiceCommand`, resolve dependencies through `build_import_invoice_use_case(...)`, and call `ImportInvoiceUseCase.execute(...)`. The command remains `dry_run=True`; successful non-review results mean canonical acceptance/evaluation only, not ERP document creation. Those accepted non-review results persist a technical `import_receipts` row keyed by company-scoped idempotency key so repeated syncs return `already_imported` instead of re-planning the same dry-run. Review creation persists immutable `ReviewClassificationEvidence` into `workbench_review_classification_evidence`, keyed by exact `company_id`, `review_id`, and `review_version`; `workbench_review_items` remains durable idempotency evidence for review-required imports. When `ODOO_WORKBENCH_PROJECTION_PUBLISH_ENABLED=true`, the composed use case commits Hub review/evidence first and then attempts best-effort Odoo projection publishing through `WorkbenchProjectionPublisher`.
