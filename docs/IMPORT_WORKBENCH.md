...
The import runtime now creates a pending Hub Workbench review for review-required results when the production composition root supplies `ReviewItemCreationService`. The production construction entrypoint is `build_import_invoice_use_case(...)` in `app/composition/imports.py`; there is currently no separate HTTP route, CLI command, or bootstrap path in `app/` that constructs `ImportInvoiceUseCase` directly. Review creation persists immutable `ReviewClassificationEvidence` into `workbench_review_classification_evidence`, keyed by exact `company_id`, `review_id`, and `review_version`. When `ODOO_WORKBENCH_PROJECTION_PUBLISH_ENABLED=true`, the composition root commits that Hub review/evidence first and then attempts best-effort Odoo projection publishing through `WorkbenchProjectionPublisher`.
