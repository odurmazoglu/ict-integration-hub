...
`EXECUTE` requires `EXECUTION_EXECUTE_ENABLED=true`, explicit `ExecutionApproval.approved_by`, `PRODUCTION_OPERATIONS_ENABLED=true` when a writer-backed strategy is present, and the configured production approval acknowledgement. Workbench `decided_by`, Odoo user identity, and projection audit fields are not execution approval. Approval acknowledgements for production writer gates are enforced by the writer policy and must not be persisted in runtime events. `VENDOR_BILL` execution may call `VendorBillWriter` to create or recover an Odoo Draft Vendor Bill. Existing-invoice `CUSTOMER_RECHARGE` execution is no-write and may only record `CUSTOMER_INVOICE` artifacts from accepted allocation evidence. Creation-mode `CUSTOMER_RECHARGE` execution may call `CustomerInvoiceWriter` only after explicit accepted `CustomerInvoiceBillingInstruction` evidence exists and `CUSTOMER_INVOICE_EXECUTE_ENABLED=true`. Missing execution opt-in, missing approval, disabled writer gates for writer-backed plans, missing billing instructions, malformed Customer Recharge allocation shape, and unsupported steps are rejected before runtime creation and before any ERP writer call.

`customer_invoice_id` is non-unique evidence, not authorization and not a one-to-one relationship. It does not authorize customer invoice creation, posting, payment, reconciliation, settlement, collection, Sales Order mutation, analytic writes, or profitability posting. The existing-invoice Customer Recharge strategy does not call Odoo or reconstruct references from display names; it relies on already accepted and previously validated allocation evidence. Customer invoice creation is authorized only by explicit version-pinned billing instruction evidence plus explicit execution approval and writer gates, not by cost allocation amounts, percentages, display fields, comments, names, incoming purchase tax mapping, or current ERP lookup.

Workbench billing evidence is Hub-owned immutable persistence. `workbench_review_billing_evidence` stores customer billing instructions before human decision submission, keyed by exact `company_id`, `review_id`, `review_version`, and `billing_key`. It is append-only: identical evidence may be replayed idempotently, but changed billing instructions for the same review version and billing key are conflicts. Billing evidence must originate from authoritative review/business workflow data. It must not be fabricated from Vendor Bill amounts, `BusinessContextAllocation.amount`, allocation percentages, purchase tax mapping, current ERP prices, pricelists, provider rereads, display text, AI, or fuzzy matching. `BusinessContextAllocation` remains vendor/source cost allocation evidence only.

Accepted customer billing evidence is stored separately in `execution_customer_billing_evidence` and is the only Customer Invoice billing source for execution planning. `SubmitReviewDecisionUseCase` reads Stage 1 by exact review, company, and expected version, then the SQLAlchemy decision writer copies the complete evidence set to Stage 2 in the same transaction as the accepted decision and execution source evidence. Stage 2 replay is exact-set append-only: removed, added, or changed billing instructions conflict, and historical evidence is never updated or deleted.

Odoo Workbench billing authoring is the authoritative Stage 1 billing instruction capture source for Customer Invoice creation. The Hub reads those rows through a read-only adapter and validates exact review/company/version identity, allocation coverage, ERP references, group consistency, and billing readiness before persistence. Missing or invalid Stage 1 evidence fails closed before accepted decision persistence, runtime creation, or ERP writer calls.
...
Review classification evidence is Hub-owned immutable persistence. `workbench_review_classification_evidence` stores the deterministic classification result that caused a review version, including explicit `NO_MATCH` and conflict evidence. It is keyed by exact `company_id`, `review_id`, and `review_version` and is persisted atomically with review creation when supplied. Historical review classification must be read from this pinned evidence and must never be recomputed from the latest Odoo Decision Rules. The reader does not call Odoo, Uyumsoft, rule repositories, classifiers, ERP writers, or runtime execution, and malformed or unsupported schema versions fail closed with safe messages.
...
Hub runtime persistence and Odoo draft creation are a distributed write boundary and cannot be committed atomically together. Recovery depends on persisted runtime state, deterministic writer idempotency, and Odoo-side duplicate lookup before create. A lost Odoo response is recovered by rerunning the same incomplete runtime step, returning the existing Draft Vendor Bill or Draft Customer Invoice, and completing the runtime without a duplicate draft. The execution runtime must not acknowledge Odoo projections, run in parallel, schedule background jobs, post accounting moves, register payments, reconcile, unlink, create non-draft customer invoices, create RFQs or Purchase Orders, or expose raw provider details.
...
- Odoo SSO
- ungated ERP writes
- browser- or Odoo-owned workflow execution
