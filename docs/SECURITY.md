...
`EXECUTE` requires `EXECUTION_EXECUTE_ENABLED=true`, explicit `ExecutionApproval.approved_by`, `PRODUCTION_OPERATIONS_ENABLED=true`, and the configured production approval acknowledgement. Workbench `decided_by`, Odoo user identity, and projection audit fields are not execution approval. Approval acknowledgements for production writer gates are enforced by the writer policy and must not be persisted in runtime events. Only pure `VENDOR_BILL` execution plans are production-capable in this slice, and they may call `VendorBillWriter` to create or recover an Odoo Draft Vendor Bill. Missing execution opt-in, missing approval, disabled writer gates, heterogeneous plans, and all non-Vendor-Bill execution steps are rejected before runtime creation and before any ERP writer call.
