...
Traceability choices in the current `ReviewDecisionCommand` are explicit user-provided `BusinessContextAllocationSet` values. Legacy `BusinessContextDecision` is retained only as historical evidence for old rows and is not accepted by the active command or API. The allocation contract supports choices such as direct Vendor Bill, RFQ/Purchase Order, existing Purchase Order matching, expense, asset, subscription, customer recharge, affiliate targeting, project cost, internal cost, or dismissal without executing those workflows in this slice. Manual Review is the unresolved state; it cannot be selected as a resolution workflow.
...
`ExecutionStepType.VENDOR_BILL` builds through `VendorBillBuilder`, writes only through `VendorBillWriter`, preserves the writer's production gates and Odoo duplicate lookup, and creates draft Vendor Bills only. `ExecutionStepType.CUSTOMER_RECHARGE` is executable in existing-invoice mode when every allocation already references an outgoing customer invoice through `customer_invoice_id`; it performs no ERP writes and records `CUSTOMER_INVOICE` artifacts with `created=false`. Creation-mode Customer Recharge execution is allowed only when accepted Stage 2 `CustomerInvoiceBillingInstruction` evidence exists, the planner creates one step per billing key, and all global plus customer invoice writer gates pass. The normal production Workbench/import flow does not yet create Stage 1 billing instructions, so Customer Invoice creation remains blocked until authoritative billing capture or Workbench authoring is implemented.
...
No RFQ, Purchase Order, expense, asset, subscription, analytic distribution, profitability posting, payment, reconciliation, unlink, or projection acknowledgement is created in this slice. Customer Invoice creation is draft-only, explicit-evidence-only, and disabled unless production execution and customer invoice writer gates are enabled.
