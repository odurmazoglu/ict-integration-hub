...
def test_uyumsoft_invoice_metadata_migration_upgrade_and_downgrade(
    ...
    source_evidence_columns = {column["name"] for column in inspector.get_columns("execution_source_invoice_evidence")}
    assert {
        "review_id",
        "company_id",
        "decision_version",
        ...
        "tax_match",
        "created_at",
    }.issubset(source_evidence_columns)
    source_evidence_indexes = {index["name"] for index in inspector.get_indexes("execution_source_invoice_evidence")}
    assert {
    ...
    billing_evidence_columns = {column["name"] for column in inspector.get_columns("workbench_review_billing_evidence")}
    assert {
        "review_id",
        "company_id",
        "review_version",
        ...
        "billing_instruction",
        "created_at",
    }.issubset(billing_evidence_columns)
    billing_evidence_indexes = {index["name"] for index in inspector.get_indexes("workbench_review_billing_evidence")}
    assert {
        "ix_workbench_review_billing_evidence_company_review_version",
        "ix_workbench_review_billing_evidence_billing_key",
    }.issubset(billing_evidence_indexes)
    billing_evidence_unique_constraints = {
        constraint["name"] for constraint in inspector.get_unique_constraints("workbench_review_billing_evidence")
    }
    ...
    classification_evidence_columns = {
        column["name"] for column in inspector.get_columns("workbench_review_classification_evidence")
    }
    assert {
        "review_id",
        "company_id",
        ...
        "conflicting_rules",
        "created_at",
    }.issubset(classification_evidence_columns)
    classification_evidence_indexes = {
    ...
    source_invoice_evidence_columns_at_head = {
        column["name"] for column in inspector.get_columns("workbench_review_source_invoice_evidence")
    }
    assert {
        "id",
        "review_id",
        ...
        "invoice",
        "created_at",
    }.issubset(source_invoice_evidence_columns_at_head)
    source_invoice_evidence_indexes_at_head = {
        index["name"] for index in inspector.get_indexes("workbench_review_source_invoice_evidence")
    }
    assert {
        "ix_workbench_review_source_invoice_evidence_company_review",
        "ix_workbench_review_source_invoice_evidence_source_invoice_id",
    }.issubset(source_invoice_evidence_indexes_at_head)
    source_invoice_evidence_unique_constraints_at_head = {
        constraint["name"]
    ...
    command.downgrade(config, "-1")
    inspector = inspect(create_engine(database_url))
    assert "workbench_review_source_invoice_evidence" not in inspector.get_table_names()
    ...
    command.downgrade(config, "-1")
    inspector = inspect(create_engine(database_url))
    assert "workbench_review_classification_evidence" not in inspector.get_table_names()
    assert "execution_customer_billing_evidence" in inspector.get_table_names()
    assert "workbench_review_billing_evidence" in inspector.get_table_names()
    ...
    command.downgrade(config, "-1")
    inspector = inspect(create_engine(database_url))
    assert "workbench_review_billing_evidence" not in inspector.get_table_names()
    assert "workbench_review_execution_evidence" in inspector.get_table_names()
    assert "execution_source_invoice_evidence" in inspector.get_table_names()
    ...
    command.downgrade(config, "-1")
    inspector = inspect(create_engine(database_url))
    assert "execution_source_invoice_evidence" not in inspector.get_table_names()
    assert "workflow_executions" in inspector.get_table_names()
    assert "workflow_execution_steps" in inspector.get_table_names()
    assert "workflow_execution_events" in inspector.get_table_names()
